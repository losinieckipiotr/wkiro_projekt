...
for iBand = 1:numScaleBands
   for iScale = 1:length(ScalesInThisBand{iBand})
     for iFilt = 1:numSimpleFilters
       ...
       if ~USECONV2
     temp = abs(imfilter(stim,sqfilter{iUFilterIndex},'symmetric','same','corr'));
     %imshow(temp)
     %pause
	 s1{iBand}{iScale}{iFilt} = temp;
	 if(~INCLUDEBORDERS)
	   temp = removeborders(s1{iBand}{iScale}{iFilt},fSiz(iUFilterIndex));
       s1{iBand}{iScale}{iFilt} = temp;
	 end
     ...
     %imshow(stim);
     temp = abs(conv2(stim,sqfilter{iUFilterIndex},'same'));
     %imshow(temp);
	 s1{iBand}{iScale}{iFilt} = temp;
	 if(~INCLUDEBORDERS)
	   s1{iBand}{iScale}{iFilt} = removeborders(s1{iBand}{iScale}{iFilt},fSiz(iUFilterIndex));
     end
     temp = im2double(s1{iBand}{iScale}{iFilt}) ./ s1Norm{fSiz(iUFilterIndex)};
	 s1{iBand}{iScale}{iFilt} = temp;
       end
     end
   end
...
for iBand = 1:numScaleBands
  for iFilt = 1:numSimpleFilters
    ...
    for iScale = 1:length(ScalesInThisBand{iBand});
      t =  max(c1{iBand}(:,:,iFilt),s1{iBand}{iScale}{iFilt});
      c1{iBand}(:,:,iFilt) = t;
    end
  end
end
...
for iBand = 1:numScaleBands
  ...
  for iFilt = 1:numSimpleFilters
      ret = maxfilter(c1{iBand}(:,:,iFilt),[0 0 poolRange-1 poolRange-1]);
      %imshow(ret);
      %pause;
    c1{iBand}(:,:,iFilt) = ret;
  end
